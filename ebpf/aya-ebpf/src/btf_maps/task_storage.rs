...
use crate::{
    bindings::{
        bpf_map_type::BPF_MAP_TYPE_TASK_STORAGE, BPF_F_NO_PREALLOC, BPF_LOCAL_STORAGE_GET_F_CREATE,
    },
    cty::c_void,
...
#[repr(transparent)]
pub struct TaskStorage<T, const F: usize = 0>(UnsafeCell<TaskStorageDef<T, F>>);

unsafe impl<T, const F: usize> Sync for TaskStorage<T, F> {}

impl<T, const F: usize> TaskStorage<T, F> {
    #[inline]
    pub unsafe fn get_or_insert(&self, inode: *mut c_void, initial: &T) -> Option<&mut T> {
    ...
    /// memory model.
    #[inline]
    pub unsafe fn get(&self, inode: *mut c_void) -> Option<&T> {
    ...
    #[inline]
    pub unsafe fn get_mut(&self, inode: *mut c_void) -> Option<&mut T> {
    ...
    /// value exists.
    #[inline]
    pub fn get_ptr(&self, inode: *mut c_void) -> Option<*const T> {
        self.get_ptr_mut(inode).map(|p| p as *const T)
    }
