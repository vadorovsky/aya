...
impl<K, V, const M: usize, const F: usize> HashMap<K, V, M, F> {
    ...
    pub unsafe fn get(&self, key: &K) -> Option<&V> {
        unsafe { get(self.0.get().cast(), key) }
    }
