        ...
        for i in 0..types.types.len() {
            ...
            match t {
                ...
                BtfType::DataSec(d) if features.btf_datasec => {
                    ...
                    // There are some cases when we don't have to fixup size:
                    //
                    // * When the compiler does indeed populate the size.
                    // * Extern-backing datasecs (e.g. `.ksyms`). They have
                    //   their size and variable offsets set at the previous
                    //   step. Further, not every extern BTF VAR has
                    //   corresponding ELF symbol preserved, so we skip all
                    //   fixups for such sections.
                    // log::error!("section infos: {section_infos:#?}");
                    // if d.size > 0 || name == ".ksyms" {
                    if d.size > 0 {
                        ...
                        for (i, e) in entries.iter_mut().enumerate() {
                            if let BtfType::Func(_) = types.type_by_id(e.btf_type)? {
                                e.btf_type = ksym_func_placeholder_id
                                    .expect("ksym_func_placeholder_id must be set");
                            }
                            log::error!("SEC MEMBER {i} btf_type: {}", e.btf_type);
                ...
                BtfType::Func(ty) => {
                    ...
                    log::error!(
                        "CHECKING FUNC {name}, linkage: {:?}, ty: {ty:?}",
                        ty.linkage()
                    );
                    if types.ksym_types.contains(&(i as u32)) {
                        log::error!("FOUND KSYM!!!");
