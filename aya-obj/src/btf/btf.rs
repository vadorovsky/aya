...
impl Btf {
    ...
    fn sanitize_program_type(
        &mut self,
        section_infos: &HashMap<String, (SectionIndex, u64)>,
        symbol_offsets: &HashMap<String, u64>,
        features: &BtfFeatures,
        enum64_placeholder_id: &OnceCell<u32>,
        filler_var_id: &OnceCell<u32>,
        types: &mut BtfTypes,
        i: usize,
    ) -> Result<(), BtfError> {
    ...
    /// Adjusts values in the program's BTF before it's loaded into the kernel.
    ///
    /// The most of fixups are inspired by [`bpf_object__sanitize_btf` in libbpf][sanitize-libbpf].
    ///
    /// [sanitize-libbpf]: https://github.com/libbpf/libbpf/blob/05f94ddbb837f5f4b3161e341eed21be307eaa04/src/libbpf.c#L2701
    pub(crate) fn sanitize_program(
        &mut self,
        section_infos: &HashMap<String, (SectionIndex, u64)>,
        symbol_offsets: &HashMap<String, u64>,
        features: &BtfFeatures,
...
impl Object {
    ...
    pub fn sanitize_btf(&mut self, features: &BtfFeatures) -> Result<Option<&Btf>, BtfError> {
        if let Some(obj_btf) = &mut self.btf {
            if obj_btf.is_empty() {
                return Ok(None);
            }
            // fixup btf
            obj_btf.sanitize_program(&self.section_infos, &self.symbol_offset_by_name, features)?;
            Ok(Some(obj_btf))
        } else {
            Ok(None)
        }
...
mod tests {
    ...
    fn test_fixup_ptr() {
        ...
        btf.sanitize_program(&HashMap::new(), &HashMap::new(), &features)
            .unwrap();
